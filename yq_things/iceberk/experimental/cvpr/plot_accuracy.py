...
pyplot.legend(('K-means', '2x PDL', '4x PDL', '8x PDL'), loc = 'lower right')
fig = pyplot.gcf()
...
# I guessed some values... will need to fill them later
all_accu = np.array(\
[[ 0.65468     ,0.          ,0.          ,0.          ,0.          ,0.          ,0.        ],
...
pyplot.xlabel('Final Dictionary Size')
pyplot.ylabel('Accuracy')
pyplot.legend(('K-means', '2x PCA', '4x PCA', '8x PCA'), loc = 'lower right')
fig = pyplot.gcf()
fig.set_size_inches([6., 4.])
...
""" old code: relative improvement
pyplot.figure()
#pyplot.plot(all_dims, np.diag(all_accu) - np.diag(all_accu))
for i in range(1, 4):
    pyplot.semilogx(all_dims[:-i], np.diag(all_accu,-i) - np.diag(all_accu)[:-i], line_formats[i], lw=2)
pyplot.xticks(all_dims, [str(c) for c in all_dims])
pyplot.grid(True)
"""
